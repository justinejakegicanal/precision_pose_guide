...
# Create a queue to handle sounds so the video doesn't freeze
audio_message_queue = queue.Queue()

# This part runs in the background to play the beeps and alarms
...
# Initialize the camera and the pose detector
video_capture = cv2.VideoCapture(0)
fitness_detector = pm.PoseDetector()

# Global variables to track reps, sets, and app states
total_rep_count = 0
...
# Function to draw text with a shadow so it's easy to read
...
while True:
    ...
    if not capture_success:
        continue
    # Standardize the resolution and find the person's pose
    ...
    # Detect which arm is more visible to track that one
    ...
    # State 2: Display the workout-specific instructions
        ...
        if selected_workout_mode == "Curl":
            workout_instructions = "CURL: Keep elbow pinned. Squeeze up!"
        elif selected_workout_mode == "Row":
            workout_instructions = "ROW: Back flat. Pull dumbbell to hip."
        elif selected_workout_mode == "Press":
            workout_instructions = "PRESS: Push weight straight up to lock out!"
    ...
    # State 3: Show a 10-second countdown before the set starts
        ...
        if remaining_seconds <= 0:
            current_software_state = "WORKOUT"
            
    # State 4: Active Workout Tracking
        ...
        if is_user_resting_status:
            ...
            if selected_workout_mode == "Curl":
                ...
                visual_progress_bar_coordinate_y = np.interp(current_joint_angle, (65, 165), (100, 650))
            elif selected_workout_mode == "Row":
                rep_completion_percentage = np.interp(current_joint_angle, (85, 155), (100, 0))
                visual_progress_bar_coordinate_y = np.interp(current_joint_angle, (85, 155), (100, 650))
            elif selected_workout_mode == "Press":
                rep_completion_percentage = np.interp(current_joint_angle, (200, 290), (100, 0))
                visual_progress_bar_coordinate_y = np.interp(current_joint_angle, (200, 290), (100, 650))
            ...
            if total_rep_count < 12:
                if rep_completion_percentage == 100:
                    ...
                    if movement_direction_flag == 0:
                        ...
                        if total_rep_count < 12:
                            audio_message_queue.put((1500, 200)) 
            ...
            cv2.rectangle(current_frame, (1100, int(visual_progress_bar_coordinate_y)), (1175, 650), ui_display_color, cv2.FILLED)
    ...
    if keyboard_input_key == ord('q'):
        break
    if current_software_state == "MENU":
        if keyboard_input_key == ord('1'):
            selected_workout_mode = "Curl"
            current_software_state = "GUIDE"
        elif keyboard_input_key == ord('2'):
            selected_workout_mode = "Row"
            current_software_state = "GUIDE"
        elif keyboard_input_key == ord('3'):
            selected_workout_mode = "Press"
            current_software_state = "GUIDE"
    elif current_software_state == "GUIDE" and keyboard_input_key == 13: 
        current_software_state = "COUNTDOWN"
        countdown_timestamp_start = time.time()
        angle_history_list = []
